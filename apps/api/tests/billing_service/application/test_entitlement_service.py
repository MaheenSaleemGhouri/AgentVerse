...
from tests.billing_service.application.test_plan_catalog_service import FakePlanRepository
...
def _service(usage: ResourceUsage) -> tuple[EntitlementService, FakeUsageRepository]:
    repo = FakeUsageRepository(usage)
    service = EntitlementService(
        catalog=PlanCatalogService(plans=FakePlanRepository([_free_plan()])),
        usage=repo,
    )
    return service, repo
