...
Mirrors `tests/orchestration_service/integration/conftest.py`: skips
rather than falling back to something that is not a real database, and
takes a function-scoped engine because pytest-asyncio gives each test
its own event loop.
"""
