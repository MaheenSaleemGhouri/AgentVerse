...
Two mechanisms, no overlap. A **Stripe promotion code** is passed through to Checkout and discounts the subscription's price at the provider. An **AgentVerse coupon** grants *account credit*, which reduces what any future invoice collects and survives plan changes, cancellation and resubscription.

Referral attribution is written server-side when the referred workspace is created, and qualifies on the referred workspace's **first successful payment** — never on signup, which would be a bounty on creating accounts.
