  ...
  "fixed z-50 flex flex-col gap-4 bg-card shadow-xl transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        ...
        left: "inset-y-0 left-0 h-full w-full border-r border-border sm:max-w-md data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left",
...
function SheetContent({
  ...
  return (
    <SheetPrimitive.Portal>
      ...
      >
        {children}
        {/* Radix's own Close — keeps the focus trap and Esc handling
            that a hand-rolled close button would silently drop. */}
        <SheetPrimitive.Close className="absolute top-4 right-4 rounded-md p-1 opacity-70 transition-opacity hover:opacity-100 focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none">
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("mt-auto flex gap-2 border-t border-border px-6 py-4", className)}
      {...props}
    />
  )
}
