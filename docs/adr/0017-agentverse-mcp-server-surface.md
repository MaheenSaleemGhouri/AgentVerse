...
### Registered as a raw `Route("/mcp", ...)`, not `Mount` at any prefix

`FastMCP`'s sub-app already declares its own route at `/mcp` internally (the SDK default `streamable_http_path`). Mounting it under `app.mount("/mcp", mcp_asgi_app())` would make a bare `POST /mcp` hit only the `Mount`'s own prefix boundary — Starlette redirects that exact case to add a trailing slash (307 to `/mcp/`) regardless of what path the child app declares internally, because the computed child `route_path` for an exact-prefix hit is an empty string, which never matches a `Route` (always `/`-prefixed). A real MCP client POSTing a streamed body to `/mcp` and not following redirects on a POST would break.

The first shipped fix mounted at the app's root instead (`app.mount("/", mcp_asgi_app())`), which does reach the sub-app's `/mcp` route with zero redirect hops — but a root `Mount` matches literally *every* path the app has not already matched (`Mount`'s path regex for prefix `""` is `"/{path:path}"`, matching any string starting with `/`), and delegates every one of them to the MCP sub-app, which then 404s internally for anything other than `/mcp`. This is invisible as long as every route in the app is registered inside `create_app()` *before* the mount — true for the app's own routers — but it silently shadows any route added to the `app` object afterward. That is exactly what several pre-existing integration tests do (`test_ip_allowlist_routes.py`, `test_custom_roles.py`, `test_resource_permissions.py`: each builds `app = create_app()` then `app.include_router(_test_router)` to add a temporary diagnostic route for the test) — those routes were silently 404ing through the MCP sub-app instead of ever being reached, a real regression caught by running the full integration suite, not by the root-mount's own tests (which only ever hit `/mcp` and genuinely-unmatched paths, both cases the root mount handles correctly by construction).

The fix: `main.py` registers the sub-app as a bare `Route("/mcp", endpoint=mcp_asgi_app())` — the exact mechanism `FastMCP` uses internally for its own `/mcp` route (`Route.__init__` treats a class-instance endpoint, which a `Starlette` sub-app is, as raw ASGI and hands it the request unmodified, with no prefix-stripping). A `Route` matches only its own literal path via a fixed regex; it can never shadow a sibling route no matter when that sibling is registered, so both the redirect problem `Mount("/mcp", ...)` had and the shadowing problem `Mount("/", ...)` had are avoided at once.
